...
public class JWTAuthorizationFilter extends BasicAuthenticationFilter {
    ...
    private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest request) {
        ...
        if (token != null) {
            // parse the token.
            String userSessionId = JWT.require(Algorithm.HMAC512(SECRET.getBytes()))
                    .build()
                    .verify(token.replace(TOKEN_PREFIX, ""))
                    .getSubject();


            if (userSessionId != null && JwtStorage.validateSession(userSessionId) && JwtStorage.validate(token)) {
                return new UsernamePasswordAuthenticationToken(userSessionId, null, new ArrayList<>());
            }
            return null;
        }
        return null;
